...
export default function AllVideoComments({ videoId: propVideoId, refType = "video", refresh = 0 }) {
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const result = await axiosInstance.get(`/v1/comments/${videoId}?refType=${encodeURIComponent(refType)}`);
        ...
        await Promise.all(
          commentsList.map(async (c) => {
            try {
              const r = await axiosInstance.get(`/v1/like/c/${c._id}`);
  ...
  const handleLikeComment = async (commentId) => {
    try {
      const res = await axiosInstance.post(`/v1/like/toggle/c/${commentId}`,
        {});
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      await axiosInstance.delete(`/v1/comments/c/${commentId}`);
