...
function VideoPage() {
  ...
  const addLike = async () => {
    try {
      setIsLiking(true);
      const res = await axiosInstance.post(`/v1/like/toggle/v/${videoId}`,
        {});
  ...
  const playVideo = async () => {
    try {
      const result = await axiosInstance.get(`/v1/videos/${videoId}`);

      setVideo(result.data.video);
      try {
        await axiosInstance.post(`/v1/users/history/${videoId}`, {});
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        const res = await axiosInstance.post(`/v1/videos/${videoId}/view`, {});
    ...
    const loadCounts = async () => {
      try {
        const [likesRes, commentsRes] = await Promise.all([
          axiosInstance.get(`/v1/like/v/${videoId}`),
          axiosInstance.get(`/v1/comments/${videoId}?refType=video`),
        ]);
  ...
  useEffect(() => {
    const loadRecommendations = async () => {
      try {
        const res = await axiosInstance.get(`/v1/videos?limit=12`);
    ...
    const loadChannelVideos = async () => {
      try {
        const channelId = video?.owner?._id || video?.owner;
        if (!channelId) return;
        const res = await axiosInstance.get(`/v1/videos?owner=${channelId}`);
        const items = res.data?.videos || [];
