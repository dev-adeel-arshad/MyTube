...
export default function WatchHistoryPage() {
  ...
  const hasShortsHistory = shortsHistory.length > 0;
  ...
  const shortsToday = shortsHistory.filter((entry) => isSameDay(entry?.watchedAt));
        ...
        <div className="history-main">
          ...
          {activePanel === "history" && (
            <div className="history-tabs" role="tablist" aria-label="History filters">
              ...
              <button
                type="button"
                className={`history-tab ${activeFilter === "shorts" ? "active" : ""}`}
                      ...
                      <div className="history-comment-meta">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (comment.refType === "short") navigate(`/shorts/${comment.refId}`);
                          }}
                        >
                          View
                        </button>
          ...
          ) : !hasHistory && !hasShortsHistory && !hasTweetHistory && !loading ? (
            ...
            <div>
              {showVideos && (
                <>
                  ...
                  {showShorts && shortsToday.length > 0 && (
                    <div className="history-group">
                      <div className="history-group-head">
                        <h3 className="history-group-title">Today</h3>
                        <span className="history-group-tag">Shorts</span>
                        <button
                          type="button"
                          className="history-group-action"
                          onClick={() => navigate("/shorts")}
                        >
                          View all
                        </button>
                      </div>
                      <div className="shorts-history-grid">
                        {shortsToday.map((entry) => (
                          <div
                            key={entry?.short?._id}
                            className="shorts-history-card"
                            onClick={() => navigate(`/shorts/${entry?.short?._id}`)}
                          >
                            <img
                              src={entry?.short?.thumbnail || "/default-thumb.svg"}
                              alt={entry?.short?.title || "Short"}
                            />
                            <div className="shorts-history-meta">
                              <h4>{entry?.short?.title || "Untitled"}</h4>
                              <span>
                                {entry?.short?.owner?.username || entry?.short?.owner?.fullname || "Creator"}
                              </span>
                            </div>
                          </div>
                        ))}
              ...
              {showShorts && (
                <div className="history-group">
                  <div className="history-group-head">
                    <h3 className="history-group-title">Shorts</h3>
                    <button
                      type="button"
                      className="history-group-action"
                      onClick={() => navigate("/shorts")}
                  ...
                  {hasShortsHistory ? (
                    ...
                    <p className="no-history">No shorts history yet.</p>
                  )}
                </div>
              )}
